# go-ruby-puppet documentation

**Ruby Puppet API adapter over the pure-Go go-puppet engine (CGO=0).**

`go-ruby-puppet/puppet` is a faithful, pure-Go (zero cgo) reimplementation of Ruby's `puppet`,
matching reference Ruby (MRI) behaviour. The module path is
`github.com/go-ruby-puppet/puppet`.
...
!!! success "Status: pure-Go, CGO=0, differential-tested"
    A faithful pure-Go port of Ruby's `puppet`, validated against reference Ruby, at 100%
...
```sh
go get github.com/go-ruby-puppet/puppet
```
...
| Repo | What it is |
| --- | --- |
| [`puppet`](https://github.com/go-ruby-puppet/puppet) | the library — Ruby's `puppet` in pure Go |
| [`docs`](https://github.com/go-ruby-puppet/docs) | this documentation site (MkDocs Material, versioned with mike) |
| [`go-ruby-puppet.github.io`](https://github.com/go-ruby-puppet/go-ruby-puppet.github.io) | the organization landing page (Hugo) |
| [`brand`](https://github.com/go-ruby-puppet/brand) | logo and brand assets |
...
Source lives at [github.com/go-ruby-puppet/puppet](https://github.com/go-ruby-puppet/puppet).
