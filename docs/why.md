...
`go-ruby-puppet/puppet` reimplements Ruby's `puppet` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-puppet/puppet` directly, with no Ruby runtime;
...
Because `puppet` is pure Go with cgo disabled, it also compiles to **WebAssembly**
